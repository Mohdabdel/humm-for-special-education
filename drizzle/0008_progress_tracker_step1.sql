...
-- Tables: measurement_definition (simplified), data_point (full)
-- NOT applied to the database by this step.

CREATE TYPE "public"."measurement_value_type" AS ENUM('count', 'percentage', 'duration_seconds', 'frequency', 'rating_scale', 'trials_correct', 'yes_no');--> statement-breakpoint
CREATE TYPE "public"."measurement_direction" AS ENUM('increase', 'decrease', 'maintain');--> statement-breakpoint
CREATE TYPE "public"."data_point_source_type" AS ENUM('direct_observation', 'session_record', 'task_performance', 'family_report', 'learner_report', 'assessment');--> statement-breakpoint
CREATE TYPE "public"."data_point_status" AS ENUM('draft', 'confirmed', 'superseded');--> statement-breakpoint
...
CREATE TABLE "measurement_definition" (
	...
	"scale_min" numeric,
	"scale_max" numeric,
	"direction" "measurement_direction" NOT NULL,
...
CREATE TABLE "data_point" (
	...
	"context_note" text,
	"status" "data_point_status" DEFAULT 'draft' NOT NULL,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL
);--> statement-breakpoint
...
ALTER TABLE "data_point" ADD CONSTRAINT "data_point_observation_id_fk" FOREIGN KEY ("observation_id") REFERENCES "public"."observation"("observation_id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "data_point" ADD CONSTRAINT "data_point_session_id_fk" FOREIGN KEY ("session_id") REFERENCES "public"."session"("session_id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
-- D-33 lesson: cross-table case derivation MUST go through SECURITY DEFINER
-- helpers so RLS policies never subquery RLS-protected tables directly.
CREATE OR REPLACE FUNCTION public.has_measurement_plan_access(_measurement_plan_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $function$
  SELECT EXISTS (
    ...
    JOIN public.goal g ON g.goal_id = mp.goal_id
    WHERE mp.measurement_plan_id = _measurement_plan_id
